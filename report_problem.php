...
<html lang="en">
	<?php
			...
			// 建立SQL字串
			//$sql = "SELECT * FROM `mms_machine_status` WHERE `Status_Machine` ='線速過低' or  `Status_Machine` ='線速過高' or  `Status_Machine` ='異常停機─緊急狀況' or `Status_Thermal`='溫度過高' ";
			$sql ="SELECT * FROM `mms_machine_status` limit 0,1777  ";
			...
			{
				if($datainfo['Status_Machine']=='線速過低' || $datainfo['Status_Machine']=='線速過高' || $datainfo['Status_Machine']=='異常停機─緊急狀況' || $datainfo['Status_Thermal']=='溫度過高')
				{
					$count_data++ ;						
				}
				if($check_time==120){
	...
	<body>
		...
		<div id="content">
			...
			<section>
				<div id="report_1" class="report_point"><a>問題報表</a></div>
				<div id="report_2"><a>生產效能分析</a></div>
